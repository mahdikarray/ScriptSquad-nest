...
export class AuthController {
  ...
  async updatePassword(@Param('email') email: string, @Body('newPassword') newPassword: string, @Res() res: any) {
    try {
      const hashedPassword = await bcrypt.hash(newPassword, 10);
      const updatedUser = await this.userService.updatePassword(email, hashedPassword);
      if (!updatedUser) {
